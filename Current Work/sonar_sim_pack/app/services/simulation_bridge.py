...
async def _generate_sonar_entities_async(
    ...
    for idx, (name, result) in enumerate(zip(company_names, raw_results)):
        eid = f"sim-{idx:04d}"
...
def simulate(
    ...
    if use_sonar:
        _gen = lambda field_names: generate_sonar_entities(
            field_names,
            domain_spec,
        ...
        _gen = lambda field_names: generate_synthetic_entities(
            field_names, domain_spec, entity_count, seed
        )
