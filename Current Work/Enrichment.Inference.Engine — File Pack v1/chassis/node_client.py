...
Usage:
    from l9.chassis.node_client import NodeClient
    client = NodeClient(url=settings.score_node_url, secret=settings.inter_node_secret)
...
class NodeClient:
    ...
    def _build(
        ...
        content_hash = hashlib.sha256(
            json.dumps(
                {"action": action, "payload": payload, "tenant": tenant}, sort_keys=True
            ).encode()
        ).hexdigest()
        ...
        env: dict[str, Any] = {
            ...
            "address": {
                ...
                "destination_node": "unknown",  # resolved by receiver
    ...
    async def _send(self, envelope: dict) -> dict:
        ...
        raise RuntimeError(f"NodeClient: {self._retries} retries exhausted") from last_exc
