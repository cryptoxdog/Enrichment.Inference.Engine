...
def propose(
    ...
    for result in batch_results:
        ...
        if hasattr(confidences_raw, "entries"):
            conf_map = {e.field_name: e.confidence for e in confidences_raw.entries.values()}
        elif isinstance(confidences_raw, dict):
        ...
        for field_name, value in final_fields.items():
            ...
            if value is not None:
                ...
                conf = conf_map.get(field_name, 0.0)
                acc.confidences.append(conf)
                sources = confidences_raw
                if isinstance(sources, dict):
                    entry = sources.get("entries", sources).get(field_name, {})
...
def apply(
    ...
    if first_node_key and isinstance(nodes, dict):
        ...
        if isinstance(node_def, dict):
            ...
            for fp in proposal_set.proposed_fields:
