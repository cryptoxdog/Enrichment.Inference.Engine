...
- domain_reader forwarded through register() -> init_handlers()
- GraphSyncClient import/init guarded to avoid hard import failures
- safe field subset used for EnrichResponse.model_validate in enrich_and_sync
"""
...
def register(kb, idem_store=None, domain_reader=None) -> None:
    ...
    try:
        graph_sync_client_class = GraphSyncClient
        if graph_sync_client_class is None:
            from app.engines.graph_sync_client import GraphSyncClient as _GraphSyncClient

            graph_sync_client_class = _GraphSyncClient

        _graph_client = graph_sync_client_class(
            graph_url=settings.graph_node_url,
            api_key=settings.inter_node_secret,
            source_node="enrichment-engine",
        )
    except Exception as exc:
        logger.warning("orchestration.graph_client_init_failed", error=str(exc))
        _graph_client = None
...
def _make_enrich_and_sync_handler(kb, idem_store):
    ...
    async def handle_enrich_and_sync(tenant: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            safe_result = {
                key: value for key, value in enrich_result.items() if key in _SAFE_FIELDS
            }
            response_obj = EnrichResponse.model_validate(safe_result)
