...
Connects: chassis handlers → enrichment → inference → graph sync → downstream packets

Called once at startup by app/main.py to register all handlers with
the chassis and bind the graph sync client.
...
def register(kb, idem_store=None, domain_reader=None) -> None:
    """
    Register all ENRICH handlers with the chassis.
    ...
    async def handle_enrich_and_sync(tenant: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if _graph_client:
            try:
                ...
                logger.info(
                    "orchestration.graph_sync",
                    entity_id=entity_id,
                    status=sync_resp.get("status"),
                )
            except Exception as exc:
                logger.warning(
                    "orchestration.graph_sync_failed",
                    entity_id=entity_id,
                    error=str(exc),
                )
        ...
        try:
            ...
            logger.debug("orchestration.packet_router_not_available")
        except Exception as exc:
            logger.warning(
                "orchestration.score_dispatch_failed",
                entity_id=entity_id,
                error=str(exc),
            )
        ...
        try:
            ...
            logger.debug("orchestration.event_emitter_not_available")
        except Exception as exc:
            logger.warning(
                "orchestration.event_emit_failed",
                entity_id=entity_id,
                error=str(exc),
            )
...
async def run_outcome_feedback(
    ...
    """
    Send a match outcome back to graph for reinforcement feedback.
