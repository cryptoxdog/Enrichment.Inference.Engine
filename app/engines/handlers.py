...
Chassis handlers bridging action -> orchestration functions.

Gap pack adaptations:
- fix KBResolver import path
- fix DomainYamlReader import path
- keep persist+graph-sync fire-and-forward
- forward domain_spec into run_convergence_loop
"""
...
async def _persist_and_sync(
    ...
    """Persist completed enrichment and trigger downstream sync hooks. Never raises."""
...
async def handle_converge(tenant: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if domain_id and node_label and _domain_reader:
        try:
            ...
            domain_spec = config.raw_spec
            if config.inference_rules_path:
                rules_path = Path(config.inference_rules_path)
                if rules_path.exists():
                    async with aiofiles.open(rules_path) as handle:
                        content = await handle.read()
                    inference_rules = yaml.safe_load(content) or []
        except Exception as exc:
            logger.warning("handle_converge.hints_load_failed", error=str(exc))
            ...
            "new_properties": [
                {
                    ...
                    "discovered_by": prop.discovered_by,
                    "confidence": prop.discovery_confidence,
                }
                for prop in proposal.new_properties
            ],
...
async def handle_simulate(tenant: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    entity_count = int(payload.get("entity_count", 20))
    seed = int(payload.get("seed", 42))
    use_sonar = bool(payload.get("use_sonar", True))
    ...
    seed_stats, enriched_stats, _seed_entities, _enriched_entities = simulate(
...
async def handle_writeback(tenant: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    crm_type = CRMType(payload.get("crm_type", "odoo"))
    ...
    mapping_path = payload.get("mapping_path", settings.crm_mapping_path)
