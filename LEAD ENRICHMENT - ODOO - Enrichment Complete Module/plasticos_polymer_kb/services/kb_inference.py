...
class KBInferenceEngine:
    ...
    def get_kb(self, polymer_code: str):
        ...
        if polymer_code not in self._kb_cache:
            ...
            kb = KB.search(
                [("polymer_type", "=ilike", polymer_code), ("active", "=", True)],
                limit=1,
            )
    ...
    def infer_quality_tier(
        self, polymer_code: str, contamination_pct: float, sorting_purity_pct: float = 0.0
    ) -> int:
        ...
        for tier in tiers:
            if tier.contamination_max_pct and contamination_pct <= tier.contamination_max_pct:
                try:
    ...
    def infer_suitable_buyer_archetypes(
        self,
        polymer_code: str,
        quality_tier: int,
        mfi: float | None = None,
    ...
    def infer_mfi_application(self, polymer_code: str, mfi: float | None) -> str:
