...
class DeterministicMatcher:
    ...
    def find_buyers(self, context) -> list:
        ...
        try:
            _logger.info(
                f"Finding buyers for polymer: {context.polymer_type}, qty: {context.quantity_kg} kg"
            )
            ...
            _logger.info(f"Found {len(matches)} qualified matches in {elapsed_ms:.2f}ms")
    ...
    def validate_technical_specs(
        self, polymer_type: str, polymer_grade: str | None, specifications: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        if polymer_type not in POLYMER_DATABASE:
            ...
            result["issues"].append(f"Unknown polymer type: {polymer_type}")
            return result
        ...
        if density:
            ...
            if not (min_d <= density <= max_d):
                ...
                result["issues"].append(f"Density {density} outside range {min_d}-{max_d} g/cm³")
        ...
        if contamination:
            ...
            if contamination > max_contam:
                ...
                result["issues"].append(f"Contamination {contamination}% exceeds max {max_contam}%")

        # Validate PCR
        pcr_pct = specifications.get("pcr_percentage")
        if pcr_pct:
            if not polymer_spec["pcr_capable"]:
                result["warnings"].append(f"{polymer_type} typically not used for PCR")
        ...
        if polymer_grade:
            if polymer_grade not in polymer_spec["common_grades"]:
                result["warnings"].append(f"Grade '{polymer_grade}' not in common grades")
    ...
    def _get_buyer_candidates(self, context) -> list:
        ...
        if context.polymer_type:
            domain.append(("polymer_interests", "ilike", context.polymer_type))
    ...
    def _score_buyer(self, buyer, context):
        ...
        total_score = technical["score"] * 0.60 + business["score"] * 0.25 + trust["score"] * 0.15
    ...
    def _score_technical_compatibility(self, buyer, context) -> dict[str, Any]:
        """
        ...
        if context.polymer_type:
            ...
            if context.polymer_type in buyer_polymers:
                polymer_compat = 1.0
                score += 0.40
                reasoning.append(f"✓ Polymer type {context.polymer_type} matches buyer interest")
            else:
        ...
        if context.contamination_pct is not None:
            ...
            if buyer_max_contam is not None:
                ...
                score += 0.15
                reasoning.append("○ No contamination limit specified by buyer")
        ...
        if context.pcr_percentage:
            ...
            if buyer_pcr_capable:
                ...
                if buyer_max_pcr and context.pcr_percentage <= buyer_max_pcr:
                    ...
                    score += 0.08
                    reasoning.append("○ Buyer PCR capable, no specific limit")
            else:
                pcr_compatible = False
                reasoning.append("✗ Buyer not capable of handling PCR content")
        ...
        if context.polymer_grade:
            ...
            if buyer_grades and context.polymer_grade in buyer_grades:
                score += 0.10
                reasoning.append(f"✓ Grade {context.polymer_grade} matches buyer preference")
            elif buyer_grades:
                # Partial credit if grade is compatible but not preferred
                score += 0.05
                reasoning.append(f"○ Grade {context.polymer_grade} acceptable but not preferred")
    ...
    def _score_business_factors(self, buyer, context) -> dict[str, Any]:
        """
        ...
        if context.quantity_kg:
            ...
            if buyer_volume_capacity:
                ...
                volume_score = 0.5
                score += 0.15
                reasoning.append("○ Buyer volume capacity not specified")
        ...
        if context.payment_terms:
            ...
            if context.payment_terms in buyer_payment_terms:
                payment_score = 1.0
                score += 0.10
                reasoning.append(f"✓ Payment terms '{context.payment_terms}' acceptable to buyer")
            else:
    ...
    def _score_trust_index(self, buyer, context) -> dict[str, Any]:
        """
        ...
        if past_transactions > 0:
            ...
            if past_transactions >= 10:
                ...
                history_score = 0.8
                score += 0.40
                reasoning.append(f"✓ Good relationship history ({past_transactions} transactions)")
            else:
                history_score = 0.5
                score += 0.25
                reasoning.append(
            ...
            history_score = 0.3
            score += 0.15
            reasoning.append("○ New buyer - no transaction history")
        ...
        if messages >= 10:
            ...
            score += 0.30
            reasoning.append(f"✓ Active communication (last 90 days: {messages} messages)")
        elif messages >= 3:
            communication_score = 0.7
            score += 0.21
            reasoning.append(f"○ Moderate communication (last 90 days: {messages} messages)")
        else:
            communication_score = 0.4
            score += 0.12
            reasoning.append(f"○ Limited recent communication (last 90 days: {messages} messages)")
        ...
        if past_transactions > 0:
            ...
            if success_rate >= 0.9:
                score += 0.20
                reasoning.append(f"✓ Excellent success rate ({success_rate * 100:.1f}%)")
    ...
    def _calculate_confidence(self, technical: dict, business: dict, trust: dict, context) -> float:
        """
    ...
    def _get_buyer_density_range(self, buyer, polymer_type: str) -> tuple[float, float] | None:
    ...
    def _get_buyer_contamination_limit(self, buyer, polymer_type: str) -> float | None:
    ...
    def _get_buyer_max_pcr(self, buyer, polymer_type: str) -> float | None:
    ...
    def _get_buyer_price_range(self, buyer, polymer_type: str) -> tuple[float, float] | None:
        """Get buyer's acceptable price range."""
        # Placeholder - implement based on buyer pricing model
        return (0.80, 1.20)  # Default range

    def _get_buyer_volume_capacity(self, buyer) -> float | None:
