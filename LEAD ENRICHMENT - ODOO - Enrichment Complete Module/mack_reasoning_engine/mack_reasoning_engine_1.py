...
class ReasoningContext:
    ...
    # Core entities
    supplier_intake_id: Optional[int] = None
    supplier_partner_id: Optional[int] = None
    buyer_partner_ids: List[int] = field(default_factory=list)
    
    # Material specifications
    polymer_type: Optional[str] = None
    polymer_grade: Optional[str] = None
    color: Optional[str] = None
    melt_index: Optional[float] = None
    density: Optional[float] = None
    contamination_pct: Optional[float] = None
...
class MackReasoningEngine:
    ...
    def execute_buyer_matching(
        ...
        try:
            ...
            augmented_matches = self._graph_service.augment_matches(
                deterministic_matches,
                context
            )
            
            # 4. Apply emotional intelligence
            ei_matches = self._apply_emotional_intelligence(
                augmented_matches,
                context
            )
            ...
            self._stats['avg_confidence'] = (
                (self._stats['avg_confidence'] * (self._stats['total_requests'] - 1) 
                 + overall_confidence) / self._stats['total_requests']
            )
            
            self._stats['avg_processing_time_ms'] = (
                (self._stats['avg_processing_time_ms'] * (self._stats['total_requests'] - 1)
                 + processing_time_ms) / self._stats['total_requests']
            )
    ...
    def execute_supplier_intake(
        ...
        try:
            ...
            normalized = self._graph_service.extract_supplier_data(
                raw_data,
                source_channel
            )
    ...
    def _apply_emotional_intelligence(
        ...
        for match in matches:
            ...
            if charisma['warmth'] < 0.5:
                match.recommendations.append(
                    "Build warmth through relationship-focused messaging"
                )
