...
def build_review_signal(changed_files: list[str]) -> tuple[bool, str]:
    ...
    if policy["review_signal"]["fail_if_contract_bound_without_companion"] and contract_bound and not companion_changed:
        fail = True
        lines.append("")
        lines.append("### Blocking finding")
        lines.append("- contract-bound files changed without companion contract/test/control updates")
    ...
    for surface_name, surface_rule in policy["surface_rules"].items():
        surface_hits = [
            path for path in changed_files if _matches_prefix(path, surface_rule["match_any_prefix"])
        ]
        if not surface_hits:
            continue
        lines.append(f"- `{surface_name}` impacted")
        companion_hits = [
            path for path in changed_files if _matches_prefix(path, surface_rule["must_update_any_prefix"])
        ]
        if not companion_hits:
            fail = True
            lines.append(f"  - missing required co-change under: {surface_rule['must_update_any_prefix']}")
