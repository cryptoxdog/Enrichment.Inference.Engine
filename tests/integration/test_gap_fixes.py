...
class TestResultStorePersistenceWiring:
    ...
    def test_enrichment_result_content_hash_is_deterministic(self):
        ...
        r1 = EnrichmentResult(
            tenant_id="t1", entity_id="e1", packet_id="p1", lineage_id="l1",
            pass_number=3, converged=True, confidence=0.91, enriched_fields=fields,
        )
        r2 = EnrichmentResult(
            tenant_id="t1", entity_id="e1", packet_id="p1", lineage_id="l1",
            pass_number=3, converged=True, confidence=0.91, enriched_fields=fields,
    ...
    async def test_result_store_save_calls_pool(self):
        ...
        result = EnrichmentResult(
            tenant_id="acme", entity_id="company_001", packet_id="pkt_x",
            lineage_id="lin_y", pass_number=2, converged=True, confidence=0.88,
    ...
    async def test_result_store_save_noop_when_no_pool(self):
        ...
        store._pool = None
        result = EnrichmentResult(
            tenant_id="t", entity_id="e", packet_id="p", lineage_id="l",
            pass_number=1, converged=False, confidence=0.5, enriched_fields={},
        )
        await store.save(result)
...
class TestGraphSyncAndScoreInvalidationHooks:
    ...
    async def test_trigger_graph_sync_posts_to_graph_url(self, monkeypatch):
        ...
        mock_settings = Settings(graph_service_url="http://graph-node:8000", perplexity_api_key="")
        ...
        with patch("httpx.AsyncClient") as MockClient:
            ...
            await graph_sync_hooks.trigger_graph_sync(
                ...
                confidence=0.92, lineage_id="lin_001", packet_id="pkt_001",
            )
    ...
    async def test_trigger_graph_sync_skips_when_no_url(self, monkeypatch):
        from app.services import graph_sync_hooks
        from app.core.config import Settings
        monkeypatch.setattr(graph_sync_hooks, "get_settings", lambda: Settings(perplexity_api_key=""))
        with patch("httpx.AsyncClient") as MockClient:
            await graph_sync_hooks.trigger_graph_sync(
                entity_id="e1", tenant_id="t1", enriched_fields={},
                confidence=0.5, lineage_id="l", packet_id="p",
            )
    ...
    async def test_invalidate_score_cache_posts_to_score_url(self, monkeypatch):
        from app.services import graph_sync_hooks
        from app.core.config import Settings
        mock_settings = Settings(score_service_url="http://score-node:8001", perplexity_api_key="")
        monkeypatch.setattr(graph_sync_hooks, "get_settings", lambda: mock_settings)
        mock_response = MagicMock()
        mock_response.raise_for_status = MagicMock()
        with patch("httpx.AsyncClient") as MockClient:
            ...
            await graph_sync_hooks.invalidate_score_cache(
                entity_id="e1", tenant_id="t1",
                lineage_id="lin_002", packet_id="pkt_002",
            )
        instance.post.assert_called_once()
...
class TestConvergeEndpointExecutesController:
    ...
    def _make_app_with_mocked_controller(self):
        ...
        with patch("app.api.v1.converge.ConvergenceController") as MockCtrl, \
            ...
            resp = client.post("/v1/converge", json={
                "tenant_id": "acme",
...
class TestCriticalImportPaths:
    ...
    def test_result_store_imports(self):
        from app.services.result_store import (
            EnrichmentResult, ResultStore,
            get_result_store, startup_result_store, shutdown_result_store,
            StorePersistenceError,
        )

    def test_graph_sync_hooks_imports(self):
        from app.services.graph_sync_hooks import (
            trigger_graph_sync, invalidate_score_cache,
        )

    def test_converge_endpoint_imports(self):
        from app.api.v1.converge import (
            router, ConvergeRequest, ConvergeResponse, converge_entity,
        )
